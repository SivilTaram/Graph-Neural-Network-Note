...
考虑到这一点，文献[1]的作者就提出要用类似直方图的方法来对每维数据分布进行建模。具体而言，请读者先通过下面的对比图来直观感受一下直方图是如何巧妙平衡**数据信息**与**数据分布**的。假设我们有100个介于`[-3,1]`的数字，如果我们直接将它们求和，如左图所示，我们完全看不出这100个数据的分布；而如果我们将`[-3, 1]`等分成4个区域，分开统计各个区域的和，我们还是可以发现一点原数据的分布特征，如右图所示。
