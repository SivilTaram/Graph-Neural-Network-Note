...
为了便于理解，我们仍然是举个例子。比如说给定一张图$G$，开始结点 $S$，对于任意一个结点 $E$，模型判断开始结点是否可以通过图游走至该结点。同样地，这也可以转换成一个对结点的二分类问题，即`可以到达`和`不能到达`。下图即描述了这样的过程：
...
从初始化的流程我们也可以看出GNN与GGNN的区别：GNN依赖于不动点理论，隐藏状态随机初始化都会收敛到不动点；GGNN则不同，好的初始化对其结果影响很大。
